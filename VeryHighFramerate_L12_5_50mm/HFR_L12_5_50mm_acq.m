...
Resource.ImageBuffer(1).numFrames = P.numSuperFrames;
...
Resource.DisplayWindow(1).numFrames = 20; %P.numSuperFrames; (?)
...
for i = 1:Resource.RcvBuffer(1).numFrames   % number of Super Frames = P.numSuperFrames
...
SeqControl(2).argument = P.time.aperture;   % 200 usecs
...
for i = 1:Resource.RcvBuffer(1).numFrames   % number of Super Frames = P.numSuperFrames
    ...
    Event(n-1).seqControl = [6,4,nsc]; % timeToNextAcq - superFrame. Also triggerOut after all frames in superFrame acquired
