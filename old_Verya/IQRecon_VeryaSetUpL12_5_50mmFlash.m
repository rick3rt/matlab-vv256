...
% thi works properly after updating on 15 March 2018 with Rik
% clear all
...
Resource.InterBuffer(1).numFrames = numOfFrames;  % one intermediate buffer needed.
...
Recon = repmat(struct('senscutoff', 0.6, ...
               ...
               'RINums', 1:3),1,Resource.RcvBuffer(1).numFrames);

for i = 1:Resource.RcvBuffer(1).numFrames
          Recon(i).IntBufDest= [1,i];
          Recon(i).RINums = (i-1)*3 + (1:3);
          Recon(i).rcvBufFrame = i;
end

% Define ReconInfo structures.
for i = 1:Resource.RcvBuffer(1).numFrames
ReconInfo((i-1)*3+(1:3)) = repmat(struct('mode', 'replaceIQ', ...  % replace IQ data.
                   'txnum', 1, ...
                   'rcvnum', (i-1)*3 +1, ...
                   'scaleFactor', 2.0, ...
                   'regionnum', 1), 1, 3);
...
ReconInfo((i-1)*3 +2).mode = 'accumIQ'; % accumulate and detect IQ data in output buffer.
ReconInfo((i-1)*3 +2).txnum = 2;
ReconInfo((i-1)*3 +2).rcvnum = (i-1)*3 +2;
ReconInfo((i-1)*3 +3).mode = 'accumIQ_replaceIntensity'; % accumulate and detect IQ data in output buffer.
ReconInfo((i-1)*3 +3).txnum = 3;
ReconInfo((i-1)*3 +3).rcvnum = (i-1)*3 +3;               
end
