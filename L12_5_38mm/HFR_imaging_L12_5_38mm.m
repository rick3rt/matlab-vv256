% HIGH FRAME RATE IMAGING FOR N FRAMES (see load_parameters function)
% Notice: 
...
SeqControl(2).argument = 0; % 2: 500 msec timeout delay
...
SeqControl(3).argument = 100; %100;  % 200 usec
SeqControl(4).command = 'timeToNextAcq';  % time between frames
SeqControl(4).argument = 9800 - 200;  % 20000 usec = 20 msec
SeqControl(5).command = 'returnToMatlab';
SeqControl(6).command = 'sync';
SeqControl(6).argument = 10000000; % 10 sec timeout for software sequencer (default is 0.5 seconds)
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).recon = 0;      % no reconstruction.
    Event(n).process = 0;    % no processing
    Event(n).seqControl = [2,nsc]; % time between syn. aper. acqs.
...
% NO jump, since one acquisition of n frames
...
frameRateFactor = P.frameRateFactor;
...
beep
answer = input('Save RcvData? [1: yes, 0: no]');
if answer == 1
    save('testRcvData.mat', 'RcvData');
    disp('DONE SAVING!')
    beep
end
