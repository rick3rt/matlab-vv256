% LOW FRAME RATE SCANNING OF WHAT TO IMAGE
% Notice: 
...
Trans.name = 'L12-5 38mm'; % has 192 elements
...
Resource.RcvBuffer(1).numFrames = P.numOfFrames;        % xx frames used for RF cineloop.
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).recon = 0;      % no reconstruction.
    Event(n).process = 0;    % no processing
    Event(n).seqControl = [4, nsc]; % time between syn. aper. acqs.
